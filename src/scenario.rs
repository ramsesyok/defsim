//! # Scenario モジュール
//! 
//! 防衛シミュレーションのシナリオ設定を管理するモジュールです。
//! 
//! このモジュールは、YAML形式のシナリオファイルの読み込み、解析、検証を行い、
//! シミュレーション実行に必要な設定情報を提供します。シナリオには、友軍・敵軍
//! の配置、戦術ポリシー、物理パラメータ、時間設定などが含まれます。
//! 
//! ## 主要機能
//! 
//! - **シナリオファイル読み込み**: YAML形式のシナリオ設定ファイルの解析
//! - **設定検証**: 読み込んだ設定値の妥当性チェック
//! - **設定構造管理**: シミュレーション用の階層化された設定データ構造
//! - **エラー処理**: 設定読み込み時の包括的なエラーハンドリング
//! 
//! ## シナリオ構成要素
//! 
//! - **メタデータ**: シナリオ名、説明、バージョン情報
//! - **シミュレーション設定**: 時間刻み（Δt）、最大時間、乱数シード
//! - **世界設定**: シミュレーション領域、高度制限、距離計算方式
//! - **指揮所設定**: 防衛対象の位置と到達判定範囲
//! - **戦術ポリシー**: 優先度計算、ランチャー選択、ミサイル誘導設定
//! - **友軍戦力**: センサーとランチャーの配置・性能
//! - **敵軍戦力**: 敵グループの出現パターンと行動パラメータ
//! 
//! ## 使用例
//! 
//! ```rust
